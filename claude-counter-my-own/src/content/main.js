(() => {
	...
	function tick() {
		...
		// Recheck limits and conversation tokens every 300 seconds (5 minutes)
		const RECHECK_MS = 300 * 1000;
		const age = now - lastUsageUpdateMs;
		if (!document.hidden && age > RECHECK_MS) {
			refreshUsage();
			refreshConversation();
		}
	}
