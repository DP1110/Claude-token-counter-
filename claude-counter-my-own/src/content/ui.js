	...
	class CounterUI {
		...
		setUsage(usage) {
			...
			if (session && typeof session.utilization === 'number') {
				...
				// Dynamically set session bar color based on percentage
				let barColor = '#10b981'; // Green: 0% to 50%
				if (width >= 95) {
					barColor = '#ef4444'; // Red: 95% to 100%
				} else if (width >= 75) {
					barColor = '#f97316'; // Orange: 75% to 95%
				} else if (width >= 50) {
					barColor = '#eab308'; // Yellow: 50% to 75%
				}
				this.sessionBarFill.style.backgroundColor = barColor;
			} else {
